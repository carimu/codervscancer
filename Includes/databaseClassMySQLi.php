<?PHP
	...
	*************************************************/
	...
	class database
	{
		...
		public function send_multi_sql($sql) { //same as send_sql() except uses 'multi_query' instead of 'query'
			if (!isset($this->mysqli))
				$this->connect();
			try {
				while ($this->mysqli->more_results() && $this->mysqli->next_result())
				{
					$this->mysqli->use_result(); //get rid of any results from previous multi_queries to allow for another multi_query
				}
				
				if (isset($this->res) && gettype($this->res) == "object")
					$this->res->free();
				if (! $this->res = $this->mysqli->multi_query($sql))
					throw new Exception("Could not send query");
			} catch (Exception $e)
			{
				echo $e->getMessage()."<BR>";
